    ...
    public class AutocomEntreInt
    {
        ...
        public List<MvcApplication8.Models.Institucion> autocomElm(string pRFC, string cct_intELIM)
        {
            ...
            using (SqlConnection cone = Conexion.conexion())
            {
                ...
                //"SELECT distinct cct,nombre FROM tablet t, institucion i WHERE t.ESTATUS ='P' and (i.CCT = t.medioprovedor or CCT =t.intitucion) and cct like '{0}%'"

                "IF(SELECT distinct ESTATUS FROM tablet WHERE medioprovedor = '" + pRFC + "' AND ESTATUS = 'M') ='M' " +
                "SELECT distinct cct, nombre FROM tablet t, institucion i WHERE t.ESTATUS = 'M' and i.CCT = t.medioprovedor and cct like 'T%' " +
                "ELSE IF(SELECT distinct ESTATUS FROM tablet WHERE proveedor = '" + pRFC + "' AND ESTATUS = 'P') = 'P' " +
                "SELECT distinct cct, nombre FROM tablet t, institucion i WHERE t.ESTATUS = 'P' and(i.CCT = t.medioprovedor or i.CCT = t.intitucion) and cct like '{0}%'" , cct_intELIM), cone);
                SqlDataReader reader = comando.ExecuteReader();
                while (reader.Read())
                {
                    MvcApplication8.Models.Institucion mde = new MvcApplication8.Models.Institucion();
